...
describe('lib', () => {
    ...
    it('works', () => {
        const element = createElement('presence', { from: 'juliet@capulet.lit/orchard', to: 'romeo@montague.lit/lane' }, createElement('status', undefined, 'Available'))
    ...
    it('supports streams stanzas', () => {
        ...
        const result = render(element)
        expect(result).toEqual('<stream:error><defined-condition xmlns="urn:ietf:params:xml:xmpp-streams"/><text xmlns="urn:ietf:params:xml:ns:xmpp-streams" xml:lang="langcode">OPTIONAL descriptive text</text>[OPTIONAL application-specific condition element]</stream:error>')
    })
})